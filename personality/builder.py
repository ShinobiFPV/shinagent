...
from personality.vernacular import build_vernacular_prompt
...
def build_system_prompt(recalled_memories: list[str] | None = None, facts: list[str] | None = None) -> str:
    ...
    # Vernacular — HOW Q2 speaks (word choice, nicknames, sentence enders,
    # slang/profanity comfort), independent of and layered on top of the
    # personality dials above (WHO Q2 is). Persists across profile/preset
    # switches via its own state file; see personality/vernacular.py.
    # Empty string (vernacular disabled, the ShinAgent default) means this
    # line is simply skipped.
    vernacular_block = build_vernacular_prompt()
    if vernacular_block:
        lines.append(vernacular_block)
        lines.append("")
