...
class IMQ2Agent:
    ...
    def reload_personality(self):
        """
        Re-read the active profile + dial state from disk (config.yaml's
        agent.active_profile, personality/profiles/<name>.yaml, and
        config/personality_state.yaml) without restarting this process.
        build_system_prompt() reads config.profile fresh on every turn, so
        this alone is enough to make the change take effect on this
        agent's next reply — no further action needed here.

        This matters specifically for a SEPARATE process from the one a
        settings change was made in (e.g. webapp/server.py's own IMQ2Agent
        subprocess) — within the SAME process, config.load_profile()/
        config.save_personality_state() already update config.profile
        in-memory directly, so this is a no-op there in practice, just a
        confirmation the state is current.
        """
        config.reload()
        log.info("Personality reloaded from disk.")
