...
def get_race_telemetry(fields: str = "summary") -> str:
    """
    Get current Forza telemetry.
    fields: 'summary' | 'tyres' | 'engine' | 'dynamics' | 'all'
    """
    try:
        from integrations.forza_telemetry import get_snapshot, is_active, CAR_CLASSES, DRIVETRAIN
        if not is_active():
            return "[race_engineer] No telemetry — is Forza running with Data Out enabled to 192.168.1.203:8000?"
        d = get_snapshot()
        if not d:
